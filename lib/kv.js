...
export async function kvDelete(key) {
  ...
  if (_driver) {
    try {
      if (typeof _driver.del === "function") {
        await _driver.del(key);
      } else if (typeof _driver.delete === "function") {
        await _driver.delete(key);
      }
    } catch (err) {
      diag("DELETE_ERROR", "driver", err?.message);
    }
    return;
  }
  const url = process.env.KV_URL;
  ...
  if (url && token) {
    try {
      await fetchWithTimeout(
        ...
        async (res) => {
          if (!res.ok) {
            diag("DELETE_ERROR", "upstash", "status:", res.status, "key:", key, "body:", await responsePreview(res));
          }
        },
      );
    } catch (err) { diag("DELETE_ERROR", "upstash", err?.message); }
    return;
  }
  ...
  if (eoKv) {
    try {
      const safeKey = sanitizeKeyForEdgeOne(key);
      await eoKv.delete(safeKey);
    } catch (err) { diag("DELETE_ERROR", "edgeone", err?.message); }
  }
}
