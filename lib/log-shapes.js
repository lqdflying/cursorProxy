...
export function uniqueResponsesToolStates(toolState) {
  const uniqueStates = new Set();
  for (const value of toolState.values()) {
    if (!value || typeof value !== "object" || !Number.isInteger(value.toolIndex)) {
      continue;
    }
    uniqueStates.add(value);
  }
  return [...uniqueStates].sort((left, right) => left.toolIndex - right.toolIndex);
}

export function incompleteResponsesToolStates(toolState) {
  return uniqueResponsesToolStates(toolState).filter((state) =>
    !state.finalArgsReceived || !state.finalArgsValid);
}
