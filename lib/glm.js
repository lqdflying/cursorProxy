...
// Detect GLM-5.2+ so reasoning_effort is forwarded upstream. Matches 5.2,
// 5.3, 6.x, 10.x, etc. Rejects older 4.x/5/5.1 and the glm-5v-turbo vision
// model (no '.' minor after the major, so the second branch does not fire).
function isGlm52PlusModel(bareModel) {
  const m = normalizeBareModel(bareModel);
  return /^glm-(?:(?:[6-9]|\d{2,})(?:[-.]|$)|5\.[2-9](?:[-.]|$))/.test(m);
}
