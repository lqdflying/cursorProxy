...
if (process.env.OPENAICOMPAT_REASONING_CACHE === "true") {
  diag("COMPATIBLE_CACHE", "env:", "OPENAICOMPAT_REASONING_CACHE", "enabled:", true);
}
...
async function injectStoredReasoning({
  ...
  if (process.env.OPENAICOMPAT_REASONING_CACHE === "true") {
    reasoningProviders.add("openaicompat");
  }
  let injectedCount = 0;
  let missedCount = 0;
  if (originalMessages && reasoningProviders.has(providerKey)) {
    ...
    for (const { i, stored, key, waitedMs, attempts } of fetched) {
      if (stored != null) {
        ...
        // Never fabricate reasoning for providers where some hosted models don't
        ...
        // "(prior reasoning unavailable)" placeholder; openaicompat points at
        // arbitrary user-chosen OpenAI-compatible endpoints whose models may
        // reject or ignore `reasoning_content`.  All three use hit-only
        // restoration: restore what we cached, skip if we missed.
        const hitOnly = providerKey === "glm"
