  ...
  if (hasMessages) {
    ...
    if (prevRespId) {
      ...
      if (nextMessages.length === 0) {
        ...
        if (openAICompatResponsesHaloCompatibleCache && toolOutputCount > 0) {
          prevRespId = null;
...
export async function retryPreviousResponseFailure({
  ...
  try {
    ...
    const isClientAbort = requestSignal?.aborted === true;
    const isTimeout = err?.name === "TimeoutError" || err?.name === "AbortError";
    const errorName = String(err?.name || "Error");
    const errorMessageLength = String(err?.message || "").length;
    log("UPSTREAM_PREV_RETRY_ERROR",
      "type:", errorName,
      "messageChars:", errorMessageLength,
      "cancelled:", isClientAbort);
    return {
      errorResponse: new Response(
        JSON.stringify({
          error: {
            message: isClientAbort
              ? "Client disconnected during previous_response_id stateless retry"
              : isTimeout
