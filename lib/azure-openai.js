...
const OPENAI_RESPONSES_REASONING_EFFORTS = new Set(["none", "minimal", "low", "medium", "high", "xhigh"]);
const AZURE_OPENAI_REASONING_EFFORTS = OPENAI_RESPONSES_REASONING_EFFORTS;
...
function openAICompatReasoningEffortEnv() {
  const raw = cleanEnvValue("OPENAICOMPAT_REASONING_EFFORT");
  if (!raw) return null;
  if (OPENAI_RESPONSES_REASONING_EFFORTS.has(raw)) return raw;
  if (!openAICompatReasoningEffortInvalidLogged) {
...
function normalizeOpenAICompatResponsesInputContent(providerKey, parsedBody) {
  ...
  if (changed) {
    diag("OAI_INPUT_NORMALIZED", "provider:", providerKey, "textParts:", textPartsFixed);
  }
