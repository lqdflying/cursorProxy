  ...
  (async () => {
    ...
    try {
        ...
        for (const line of lines) {
          ...
          if (data === "[DONE]") {
            ...
            if (providerKey === "azureopenai" || openaiCompatResponses) {
              ...
              await flushDeferredGpt56ToolChunks("unexpected_eof");
          ...
          try {
            ...
            if ((providerKey === "azureopenai" || openaiCompatResponses) && (currentResponsesEvent || json?.type)) {
              ...
              if (responsesEvent === "response.failed") {
                ...
                await flushDeferredGpt56ToolChunks("response_failed");
                await writeTerminalError({
                  message: errMessage,
                  type: errType,
                  code: errCode,
              ...
              if (mapped) {
                ...
                if (responsesEvent === "response.incomplete") {
                  ...
                  await flushDeferredGpt56ToolChunks("response_incomplete");
                  await writeTerminalError({
      ...
      if (requestCancelled) {
        ...
        if (!doneSeen) {
          try {
            await flushDeferredGpt56ToolChunks("cancelled");
            await writeTerminalError({
      ...
      if (!doneSeen && !timedOut && !requestCancelled && originalMessages && hasReasoningValue(accReasoning)) {
        const reasoningChars = reasoningSize(accReasoning);
      ...
      try {
        ...
        noteFailureStage("write");
        if (!requestCancelled && terminalOutcome !== "pipeline_error") {
      ...
      await cacheAzResponseId();
      if (!timedOut && !requestCancelled) await cacheClaudeThinking();
