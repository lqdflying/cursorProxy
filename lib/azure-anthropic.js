...
const ANTHROPIC_PROVIDERS = new Set(["azureanthropic", "anthropiccompat"]);

function remapAnthropicInput(providerKey, parsedBody) {
  if (!ANTHROPIC_PROVIDERS.has(providerKey) || !parsedBody) {
...
function normalizeAnthropicContentTypes(providerKey, parsedBody) {
  if (!ANTHROPIC_PROVIDERS.has(providerKey) || !parsedBody?.messages) {
    return { parsedBody, changed: false };
  }
...
function sanitizeAzureAnthropicBody(providerKey, parsedBody) {
  ...
  // Azure-specific env var defaults only apply to azureanthropic, not anthropiccompat
